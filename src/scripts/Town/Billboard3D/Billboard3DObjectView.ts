...
export class Billboard3DObjectView extends GameObjects.GameObject {
    ...
    private resetOpenTweenProductAndRandomProductGroup() {
        const randomBillboardIndex = this.addZeroPad(this.randomIntFromInterval(1, 3), 2)
        ...
        this.onOpenProductTweenChain = this.scene.tweens.chain({
            tweens: [
                ...
                },
                {
                    ...
                    ease: 'cubic.inout',
                    onComplete: () => {
                        this.showProductTimerSubscription = timer(Billboard3DObjectView.DELAY_SHOW_PRODUCT).subscribe(onComplete => {
